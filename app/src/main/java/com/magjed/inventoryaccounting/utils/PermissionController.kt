 ...
 * This class controls the necessary permission [mPermissionName] to be given before executing
 * [mOnPermissionGranted] function.
 * Important: this class should be instantiated in [AppCompatActivity.onCreate] method since
 * it uses [ActivityResultLauncher] under the hood.
 *
 * @param mActivity is the activity to lifecycle of which this class will be tied.
 * @param mPermissionName is the permission you want to request.
 * @param mOnPermissionGranted is the action you want to perform (invoke) after the
 * permission is granted.
 * @param mOnPermissionDenied is the action you want to be performed when the permission
...
class ActivityPermissionsController(
  ...
  override fun onCreate(owner: LifecycleOwner) {
    mPermissionRequestLauncher =
      mActivity.registerForActivityResult(ActivityResultContracts.RequestPermission(), this)
  }
  ...
  fun checkPermissionOrIgnore(): Boolean {
    val permGranted =
      mActivity.checkSelfPermission(mPermissionName) == PackageManager.PERMISSION_GRANTED
    if (permGranted) mOnPermissionGranted()
    return permGranted
  }
